...
from pydantic import BaseModel, Field
from pydantic import BaseModel, Field
>>>>>>> 27264873 (isang malupitang pagbabago - without the giant CSVs)
...
DESTINATIONS = [
    {
        ...
        "description": "Explore a beautiful resort stay. This mock description allows the destination data flow to be tested end-to-end without loading a 1GB CSV."
    }
    for i, img_url in enumerate(DESTINATION_IMAGES)
]
...
<<<<<<< HEAD
=======
# 3. Wake up the MLR Price Brain (Runs instantly on boot)
print("Loading MLR Price Model...")
PRICE_BUNDLE_PATH = BASE_DIR / "model" / "price_model_bundle.joblib"
price_bundle = joblib.load(PRICE_BUNDLE_PATH)

price_model = price_bundle["model"]
price_features = price_bundle["features"]
monthly_weather = price_bundle["monthly_weather"]
