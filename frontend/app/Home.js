...
export default function Home() {
  ...
  const searchUrl = `/destinations?startDate=${startDate}&endDate=${endDate || startDate}&date=${startDate}&guests=${guests}`;
      ...
      <div className="relative z-30 mx-auto -mt-12 w-[92%] max-w-4xl rounded-full border border-gray-200 bg-white p-2 sm:p-3 text-slate-900 shadow-[0_18px_40px_rgba(15,23,42,0.12)]">
        <div className="flex flex-col sm:flex-row gap-2 items-center px-2">
          ...
          <div className="flex-none w-full sm:w-auto mt-2 sm:mt-0 pl-2">
            <button type="button" disabled={!startDate || !endDate || endDate < startDate} onClick={handleSearch} className="flex w-full items-center justify-center gap-2 rounded-full bg-[#b9f0c8] px-8 py-4 sm:py-5 shadow-sm transition hover:scale-105 active:scale-95 text-teal-950 disabled:cursor-not-allowed disabled:opacity-50">
